...
func NewOpenAIClient(cfg ConfigOpenAI, optFns ...func(client *clientOpenAI)) (server.ClientInputToGraph, error) {
	c := clientOpenAI{
		httpClient:   nil,
		token:        cfg.Token,
...
func WithHTTPClientOpenAI(c server.HttpClient) func(o *clientOpenAI) {
...
func (c *clientOpenAI) Do(ctx context.Context, prompt string) (server.DiagramGraph, error) {
	...
	if err != nil {
		return server.DiagramGraph{}, server.Error{
			Service:                   server.ServiceOpenAI,
...
func (c *clientOpenAI) decodeResponse(ctx context.Context, respBytes []byte) (server.DiagramGraph, error) {
	var resp openAIResponse
	if err := json.Unmarshal(respBytes, &resp); err != nil {
		return server.DiagramGraph{}, server.Error{
			Service:                   server.ServiceOpenAI,
			Stage:                     server.StageDeserialization,
			Message:                   err.Error(),
			ServiceResponseStatusCode: 0,
		}
	}
...
func (c *clientOpenAI) requestHandler(ctx context.Context, payload openAIRequest) ([]byte, error) {
	...
	if err != nil {
		return nil, server.Error{
			Service:                   server.ServiceOpenAI,
			Stage:                     server.StageSerialization,
			Message:                   err.Error(),
			ServiceResponseStatusCode: 0,
		}
	}
