...
@enduml`,
		},
		{
			...
			args: args{
				graph: server.DiagramGraph{
					Nodes: []*server.Node{{ID: "0", Group: "0"}},
				},
			},
			want: `@startuml
!include https://raw.githubusercontent.com/plantuml-stdlib/C4-PlantUML/master/C4_Container.puml
...
func TestNewPlantUMLClient(t *testing.T) {
	...
	tests := []struct {
		name string
		args args
		want server.ClientGraphToDiagram
	}{
		{
			name: "default",
			args: args{},
...
func Test_clientPlantUML_Do(t *testing.T) {
	...
	type args struct {
		ctx context.Context
		v   server.DiagramGraph
	}
	tests := []struct {
		name    string
		fields  fields
		args    args
		want    server.ResponseDiagram
		wantErr bool
	}{
		{
			name: "happy path",
