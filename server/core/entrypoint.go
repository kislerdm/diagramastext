...
func InitEntrypoint(ctx context.Context, clientSecretsManager contract.ClientSecretsmanager) (
	...
	clientModelInference, clientStorage, err := initCoreClients(ctx, clientSecretsManager)
	if err != nil {
		return nil, err
	}

	return func(ctx context.Context, inquery contract.Inquiry, diagramImplementationHandler contract.DiagramHandler) (
		[]byte, error,
	) {
		if err := inquery.ValidatePrompt(); err != nil {
			return nil, err
		}
